...
export class CalculatorService {
  ...
  dividir({ number1, number2 }: DividirCalculatorDTO): number | string {
    if (number1 < number2)
      throw new BadRequestException('O resultado não pode ser menor que 1');
    return number1 / number2;
  }
  ...
  subtrair({ number1, number2 }: SubtrairCalculatorDTO): number | string {
    if (number1 < number2)
      throw new BadRequestException('O resultado não pode ser menor que 0');
    return number1 - number2;
  }
  ...
  multiplicar({ number1, number2 }: MultiplicarCalculatorDTO): number | string {
    ...
    if ([number1, number2].includes(1))
      throw new BadRequestException(
        'O resultado não pode ser igual ao number1 ou number2',
      );
